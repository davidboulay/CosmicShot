...
class Editor(Gtk.Window):
    ...
    def _build_ui(self):
        ...
        zoom_out.connect("clicked", lambda *_: self.set_zoom(self._zoom / 1.25))
        zoom_fit = Gtk.Button.new_from_icon_name("zoom-fit-best-symbolic", Gtk.IconSize.BUTTON)
        zoom_fit.set_tooltip_text("Fit (Ctrl+0)")
        zoom_fit.connect("clicked", lambda *_: self.reset_zoom())
        zoom_in = Gtk.Button.new_from_icon_name("zoom-in-symbolic", Gtk.IconSize.BUTTON)
        zoom_in.set_tooltip_text("Zoom in (+)   ·   Ctrl+scroll; scroll to pan")
        zoom_in.connect("clicked", lambda *_: self.set_zoom(self._zoom * 1.25))
